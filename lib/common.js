import { CART_UPDATE_EVT, pubsub } from "./algoliaConfig";
 ...
 * @param {*} item
 */
export function addProductToLocalStorageCart(item) {
  const items = JSON.parse(localStorage.getItem('cartItems')) || [];
  const existingItem = items.find((i) => i.objectID === item.objectID);
  if (existingItem) {
  ...
  // Propagate message to refresh events
  refreshGlobalCart();
}

/**
 * Propagate update on cart.
 */
export function refreshGlobalCart() {
  pubsub.publish(CART_UPDATE_EVT, { refresh: true });
}